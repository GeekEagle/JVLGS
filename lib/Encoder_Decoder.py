...
class FPNDecoder(nn.Module):
    def __init__(self, out_channels=32, num_classes=1):
        super().__init__()
        self.out_channels = out_channels

        # Smoothing/refinement layers after fusion
    ...
    def forward(self, features):
        # Features: List of [B, C, H, W] tensors from encoder
        f1, f2, f3 = features  # (44, 44), (22, 22), (11, 11)

        # Step 2: Top-down pathway with upsampling and fusion
        # Upsample p3 (11, 11) to (22, 22) and fuse with p2
        p3_up = F.interpolate(f3, size=f2.shape[2:], mode='bilinear', align_corners=False)
        p2_fused = self.smooth_convs[1](f2 + p3_up)  # Addition + refinement

        # Upsample p2_fused (22, 22) to (44, 44) and fuse with p1
        p2_up = F.interpolate(p2_fused, size=f1.shape[2:], mode='bilinear', align_corners=False)
        p1_fused = self.smooth_convs[0](f1 + p2_up)  # Addition + refinement

        # Step 3: Refine the highest-resolution feature
        p1_refined = self.smooth_convs[2](p1_fused)

        # Step 4: Upsample to original resolution and predict mask
        mask = self.final_conv(p1_refined)  # (B, 1, 44, 44)
        mask = F.interpolate(mask, size=(352, 352), mode='bilinear', align_corners=False)
        return mask
...
class Backbone(nn.Module):
    def __init__(self, channel=32, pvtv2_pretrained=True, imgsize=224):
        ...
        self.decoder = FPNDecoder(out_channels=self.channel)
    def forward(self, x):
        fmap = self.encoder(x)
